...
通过元数据，以及m3u8/mpd链接软件可以转换出分段链接，最终实现下载，也就是说软件所需要的链接是存放**元数据**的链接
...
发现显示在你眼前的是一些看不到的**乱码**，恭喜你，你下载的是分段链接
...
当然有，你下载这个分段链接后，使用`mediainfo`这一类工具查看其属性，如果有时间、码率这些信息，并且这个文件**只有几KB**大小
